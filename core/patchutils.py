...
def cluster_patches_withdata(data, patch_size, n_clusters, channel_count, repeat, stride, location=False):
    ...
    #faiss.write_index(kmeans.index,"kmeans_flt_imgnet_resnet_k"+str(patch_size[0])+"_s0_c"+str(n_clusters)+"_sc.index")
    if location:
        return kmeans, loc
    else:
        return kmeans
...
def cluster_patches_singleshot_fullnet_mnist(train_patch_loader, patch_size, n_clusters, channel_count, repeat, stride, getdata=False, location=False):
    kmeans = faiss.Kmeans(d=(patch_size[0]*patch_size[1]), k=n_clusters, niter=200, nredo=repeat, verbose=True)
    #kmeans = MiniBatchKMeans(n_clusters=n_clusters, random_state=0, verbose=True)
    buffer = []
    #t0 = time.time()
    # The online learning part: cycle over the whole dataset 6 times
    index = 0
    pretrained_model = "../../mnist/mnist_v0.pt"
    ...
    for _ in range(repeat):
        ...
        with torch.no_grad():
             for entry in train_patch_loader:
                ...
                # lets get the patches from the first conv layer 
                fms_orignet = extract_lowernet(mnist_model, 6)
                #for name, layer in fms_orignet.named_modules():
                #    print(name, layer)
                cond2d_orignet = fms_orignet(img_src)
                for ch in range(ch_l1):
                    fm_squeezed = cond2d_orignet[:,ch,:,:]
                    img = np.squeeze(fm_squeezed)
                    if location:
                ...
                # lets get the patches from the second conv layer 
                fms_orignet = extract_lowernet(mnist_model, 4)
                #for name, layer in fms_orignet.named_modules():
                #    print(name, layer)
                cond2d_orignet = fms_orignet(img_src)
                for ch in range(ch_l2):
                    fm_squeezed = cond2d_orignet[:,ch,:,:]
                    img = np.squeeze(fm_squeezed)
                    if location:
                        _, data, loc = extract_patches(img, patch_size, True, stride)
                    ...
                    index += 1
                        ...
                        #kmeans.partial_fit(data)
                    if index % (channel_count*200) == 0:
                        print('Patch Clustering: Patch extraction for conv2 FMs happenning  of %4i out of %i'
                              % (index, repeat * len(train_patch_loader) * channel_count))
...
def fm_to_symbolic_fm(fm, n_clusters, kmeans, centroid_lut,  patch_size, stride, channel_count, ana=False, multi=False, instr=False, randomize=False, rlevel=None, rbalance=True, pdf=None):
    ...
    for ch in range(channel_count):
        ...
        if multi:
            ...
            distances, clusters = kmeans.search(data.astype(np.float32), rlevel+1)
        else:        
        ...
        for center in clusters:
            ...
            if randomize:
                if rbalance:
                    r = np.random.random_integers(0,rlevel)
                else:
                    r = np.random.random_integers(rlevel - 3 , rlevel)
                center_patch = centroid_lut[center[r]]
            else:
